...
#else
        float4 tangentWS = float4(0.0, 0.0, 0.0, 1.0);
#endif
...
#if defined(VARYINGS_NEED_VIEWDIRECTION_WS)
        output.viewDirectionWS = SafeNormalize(GetWorldSpaceViewDir(positionWS));
#endif

#if defined(LIGHTMAP_ON)
        output.lightmapUV = input.uv1.xy * unity_LightmapST.xy + unity_LightmapST.zw;
#else
        output.sh = SampleSH(normalWS);
#endif
